...
class EmpleoFacilSupabaseUser extends BaseAuthUser {
  EmpleoFacilSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> empleoFacilSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EmpleoFacilSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
